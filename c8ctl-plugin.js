 ...
 *
 * Usage:
 *   c8ctl c8run start [<version>] [--debug]
 *   c8ctl c8run stop  [<version>]
 */
...
export const metadata = {
  name: 'c8run',
  description: 'Download, start, and stop a local Camunda 8 cluster (c8run)',
  commands: {
    'c8run': {
      description:
        'Manage local Camunda 8 cluster — use "c8ctl c8run start [version]" or "c8ctl c8run stop"',
    },
  },
};
...
async function startC8Run(config, debug = false) {
  ...
  if (existsSync(pidFile)) {
    ...
    logger.info('Use "c8ctl c8run stop" to stop it first.');
    return;
  }
  ...
  logger.info('c8run process launched, waiting for cluster to be ready...');
...
async function stopC8Run(config) {
  ...
  if (!markerExists && installedVersions.length === 0) {
    logger.warn(
      'No running c8run process found (use "c8ctl c8run start" to start one).',
    );
    return;
  }
...
export const commands = {
  'c8run': async (args) => {
    ...
    if (!parsed.subcommand || !['start', 'stop'].includes(parsed.subcommand)) {
      console.log('Usage:');
      console.log('  c8ctl c8run start [<version>] [--debug]');
      console.log('  c8ctl c8run stop  [<version>]');
      console.log('');
      ...
      console.log('  c8ctl c8run start              # Start latest stable (8.8)');
      console.log('  c8ctl c8run start 8.9.0-alpha5 # Start specific version');
      console.log('  c8ctl c8run stop');
      return;
    }
    ...
    if (parsed.subcommand === 'start') {
      try {
        ...
        logger.error(`Failed to start c8run: ${error}`);
        process.exit(1);
      }
      ...
      try {
        ...
        logger.error(`Failed to stop c8run: ${error}`);
        process.exit(1);
      }
    }
  },
